...
export default defineConfig(({ mode }) => {
  ...
  const crawleeApiTarget = env.VITE_CRAWLEE_API_TARGET || "http://127.0.0.1:8000"
  return {
    // 开发或打包构建时用到的公共基础路径
    ...
    server: {
      ...
      // 反向代理
      proxy: {
        // Crawlee Platform FastAPI（与 dev 环境 .env 中 VITE_SKIP_MOCK_LOGIN 配套；无需 Apify mock）
        "/api": {
          target: crawleeApiTarget,
          changeOrigin: true
        },
        "/health": {
          target: crawleeApiTarget,
