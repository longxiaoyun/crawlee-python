...
async function scanUsedSvgNames(options) {
  ...
  const code = await Promise.all(filenames.map(async (f) => {
    const code2 = await fs.readFile(path.resolve(process2.cwd(), f), "utf-8");
    return code2;
  }));
  function internalScanStrategy() {
    ...
    if (scanStrategy === "component") {
      ...
      for (const iconDir2 of iconDirs) {
        allSvgNames = allSvgNames.concat(fg.sync(["**/*.svg"], { cwd: iconDir2 }).map((n) => symbolIdFormatter(n, prefix)).sort((a, b) => b.length - a.length));
      }
...
async function compileSvgFiles(options, usedSvgNames) {
  ...
  for (const dir of iconDirs) {
    const svgNames = fg2.sync(["**/*.svg"], { cwd: dir });
    for (const svgName of svgNames) {
      await addSymbol(
        svgCompiler,
        svgName.replace("/", path2.sep),
        options,
        dir,
        symbolIds,
        usedSvgNames
  ...
  function initSvgCompier(options2) {
    ...
    return new Sprite({
      ...
      shape: {
        id: {
          generator(name) {
            return symbolIdFormatter(name.replace(path2.sep, "/"), prefix || "");
          }
        }
      }
    });
...
async function addSymbol(svgCompiler, svgName, options, iconDir, symbolIds, usedSvgNames) {
  ...
  const svgPath = path2.resolve(iconDir, svgName);
  const symbolId = symbolIdFormatter(svgName.replace(path2.sep, "/"), prefix);
  const svgContent = await fs2.readFile(svgPath, { encoding: "utf-8" });
...
async function optimizeSvg(source, preserveColor, optimizeOptions) {
  const { data: optimizedSvgContent } = await optimize(source, optimizeOptions);
...
function genDts(symbolIds, options) {
  ...
  const dtsPath = path3.resolve(options.dtsDir, "./svg-component.d.ts");
  const globalDtsPath = path3.resolve(options.dtsDir, "./svg-component-global.d.ts");
  if (isVue) {
    fs3.writeFile(
      dtsPath,
...
async function compileVueTemplate(template2, vueMajorVersion) {
  const searchPath = path3.resolve(process3.cwd(), "./virtual");
  ...
  const pkg = compilerInVue ? await importModule("vue/compiler-sfc") : await importModule("@vue/compiler-sfc").catch(() => {
    throw new Error(
...
function notifyClientUpdate(sprite, { ws, config }) {
  config.logger.info(
    colors2.green(`unplugin-svg-component reload`),
